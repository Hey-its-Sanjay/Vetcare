...
// If user is already logged in as patient, redirect to dashboard
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        $query = "SELECT * FROM patients WHERE (username = ? OR email = ?) LIMIT 1";
        ...
        if ($result->num_rows === 1) {
            ...
            if (true || $patient['email_verified'] == 1) {
                // Verify password
